...
css_files=['styles.css','responsive-v12.css','responsive-v13.css','responsive-v14.css','responsive-v15.css','responsive-v16.css','responsive-v17.css']
js_files=['app.js','v11-overrides.js','responsive-v12.js','responsive-v13.js','responsive-v14.js','responsive-v15.js','responsive-v16.js','responsive-v17.js']
for f in css_files:
...
summary={'version':'1.7','total':len(results),'pass':sum(x['pass'] for x in results),'fail':sum(not x['pass'] for x in results),'results':results}
(root/'RESPONSIVE_QA_RESULTS_v1.7.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
md=[f"# Responsive Browser QA v1.7", "", f"**TOTAL={summary['total']} PASS={summary['pass']} FAIL={summary['fail']}**", "", "Executed against the effective load chain through `responsive-v17.js` using headless Chromium.", "", "| Result | Check | Detail |","|---|---|---|"]
for x in results:
    detail=x['detail'].replace('|','\\|').replace('\n',' ')
    if len(detail)>160: detail=detail[:157]+'...'
    md.append(f"| {'PASS' if x['pass'] else 'FAIL'} | {x['name']} | {detail} |")
(root/'RESPONSIVE_BROWSER_QA_v1.7.md').write_text('\n'.join(md)+'\n',encoding='utf-8')
