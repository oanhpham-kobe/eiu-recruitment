...
css_files=['styles.css','responsive-v12.css','responsive-v13.css','responsive-v14.css','responsive-v15.css','responsive-v16.css']
js_files=['app.js','v11-overrides.js','responsive-v12.js','responsive-v13.js','responsive-v14.js','responsive-v15.js','responsive-v16.js']
for f in css_files:
...
with sync_playwright() as p:
    ...
    # Candidate-level batch all-or-nothing. c6 valid + c8 inactive invalid -> no partial mutation.
    ...
    check('Bulk invalid member rolls back whole batch',before==after,f'{before}->{after}')
    # c6 only succeeds.
    page.evaluate("state.selectedIds=new Set(['c6']);render()");page.wait_for_timeout(20)
    page.locator('.toolbar-left').get_by_role('button',name='Status').click();page.locator('.status-popover [role=menuitem]').filter(has_text='Đã đọc').click();page.wait_for_timeout(35)
    check('Bulk valid Candidate updates latest Submission only',page.evaluate("latestSubmissionV16(candidate('c6')).status")=='READ')
    ...
    # Candidate CV-required + staged document semantics.
...
summary={'version':'1.6','total':len(results),'pass':sum(x['pass'] for x in results),'fail':sum(not x['pass'] for x in results),'results':results}
(root/'RESPONSIVE_QA_RESULTS_v1.6.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
md=[f"# Responsive Browser QA v1.6", "", f"**TOTAL={summary['total']} PASS={summary['pass']} FAIL={summary['fail']}**", "", "Executed against the effective load chain through `responsive-v16.js` using headless Chromium.", "", "| Result | Check | Detail |","|---|---|---|"]
for x in results:
    detail=x['detail'].replace('|','\\|').replace('\n',' ')
    if len(detail)>160: detail=detail[:157]+'...'
    md.append(f"| {'PASS' if x['pass'] else 'FAIL'} | {x['name']} | {detail} |")
(root/'RESPONSIVE_BROWSER_QA_v1.6.md').write_text('\n'.join(md)+'\n',encoding='utf-8')
