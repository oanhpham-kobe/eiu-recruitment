...
for f in ['styles.css','responsive-v12.css','responsive-v13.css','responsive-v14.css','responsive-v15.css']:
    html=html.replace(f'<link rel="stylesheet" href="{f}" />',f'<style>{(root/f).read_text(encoding="utf-8")}</style>')
for f in ['app.js','v11-overrides.js','responsive-v12.js','responsive-v13.js','responsive-v14.js','responsive-v15.js']:
...
with sync_playwright() as p:
    ...
    # v1.5 anchored status dropdown + dismiss behavior
    page.set_viewport_size({'width':430,'height':844})
    page.evaluate("state.role='admin';state.lang='vi';state.page='interview';state.selectedIds.clear();render()")
    page.wait_for_timeout(30)
    badge=page.locator('.interview-table .status-clickable').first
    bb=badge.bounding_box()
    badge.click(position={'x':3,'y':max(2,bb['height']/2)})
    page.wait_for_timeout(30)
    menu=page.locator('.status-popover')
    mb1=menu.bounding_box()
    expected_left=max(12,min(bb['x'],430-mb1['width']-12))
    check('Status menu anchored to badge, not pointer X',abs(mb1['x']-expected_left)<=1,f"badge={bb} menu={mb1}")
    check('Status trigger exposes expanded state',badge.get_attribute('aria-expanded')=='true',str(badge.get_attribute('aria-expanded')))
    page.keyboard.press('Escape');page.wait_for_timeout(25)
    check('Escape closes status menu',page.locator('.status-popover').count()==0)
    check('Escape restores focus to badge','status-clickable' in (page.evaluate('document.activeElement?.className') or ''),str(page.evaluate('document.activeElement?.className')))
    badge=page.locator('.interview-table .status-clickable').first
    bb=badge.bounding_box();badge.click(position={'x':bb['width']-3,'y':max(2,bb['height']/2)});page.wait_for_timeout(25)
    mb2=page.locator('.status-popover').bounding_box()
    check('Different click position keeps same anchored menu X',abs(mb2['x']-mb1['x'])<=1,f"first={mb1['x']} second={mb2['x']}")
    page.locator('h1').click();page.wait_for_timeout(25)
    check('Outside click dismisses without selection',page.locator('.status-popover').count()==0)
    badge=page.locator('.interview-table .status-clickable').first
    before=page.evaluate("findRound('r12').status")
    badge.click();page.wait_for_timeout(20);badge.click();page.wait_for_timeout(20)
    check('Same badge toggles dropdown closed',page.locator('.status-popover').count()==0)
    check('Dismiss/toggle does not change status',page.evaluate("findRound('r12').status")==before,page.evaluate("findRound('r12').status"))

    check('No JS/console errors',len(errors)==0,str(errors))
    browser.close()

(root/'RESPONSIVE_QA_RESULTS_v1.5.json').write_text(json.dumps(results,ensure_ascii=False,indent=2),encoding='utf-8')
