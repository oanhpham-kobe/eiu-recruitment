...
js_files=['app.js','v11-overrides.js','responsive-v12.js','responsive-v13.js','responsive-v14.js','responsive-v15.js','responsive-v16.js','responsive-v17.js','responsive-v18.js','responsive-v19.js']
for f in css_files:
...
check('Responsive README/VERSION authority v1.9 + Full v1.14',readme.startswith('# EIU Recruitment — Responsive Clickable Prototype v1.9') and 'Full Handover **v1.14**' in readme and 'Business baseline: **Full Handover v1.14**' in version)
...
summary={'version':'1.9','total':len(results),'pass':sum(x['pass'] for x in results),'fail':sum(not x['pass'] for x in results),'results':results}
if not a.no_write:
    (root/'RESPONSIVE_QA_RESULTS_v1.9.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
md=[f"# Responsive Browser QA v1.9", "", f"**TOTAL={summary['total']} PASS={summary['pass']} FAIL={summary['fail']}**", "", "Executed against the effective load chain through `responsive-v19.js` using headless Chromium.", "", "| Result | Check | Detail |","|---|---|---|"]
for x in results:
    detail=x['detail'].replace('|','\\|').replace('\n',' ')
    if len(detail)>160: detail=detail[:157]+'...'
    md.append(f"| {'PASS' if x['pass'] else 'FAIL'} | {x['name']} | {detail} |")
if not a.no_write:
    (root/'RESPONSIVE_BROWSER_QA_v1.9.md').write_text('\n'.join(md)+'\n',encoding='utf-8')
