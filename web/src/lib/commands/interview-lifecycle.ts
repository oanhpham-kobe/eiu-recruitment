...
function safeMessage(code: string): string {
  const messages: Record<string, string> = {
    ...
    LATEST_ROUND_REQUIRED: "Chỉ vòng phỏng vấn mới nhất được phép thực hiện thao tác này.",
    ...
    USER_INACTIVE_NOT_SELECTABLE: "Người dùng đã ngừng hoạt động và không thể chọn.",
...
async function actorContext(deps: InterviewCommandDeps): Promise<ActorContext | null> {
...
async function executeRpc<T>(
  ...
  if (error) {
    ...
    return {
      success: false,
      error: { code: "INTERNAL_ERROR", message: "Không thể hoàn tất thao tác." },
    };
  }
...
async function withPermission<T>(
  ...
  if (!ctx)
    return { success: false, error: { code: "UNAUTHENTICATED", message: safeMessage("UNAUTHENTICATED") } };
...
export async function createNextInterviewRound(
  ...
  if (!UUID.test(input.applicationId) || !UUID.test(input.idempotencyKey)) return invalid("Application hoặc idempotency key không hợp lệ.");
...
export async function saveInterviewSchedule(
  ...
  if (!UUID.test(input.interviewId) || !positiveVersion(input.expectedVersion) || !UUID.test(input.idempotencyKey)) return invalid("Interview, version hoặc idempotency key không hợp lệ.");
  if ((input.startAt === null) !== (input.endAt === null)) return invalid("Giờ bắt đầu và kết thúc phải được nhập cùng nhau.");
  return withPermission(deps, ["interviews.manage"], (ctx) =>
...
export async function changeInterviewScheduleStatus(
  ...
  if (!UUID.test(input.interviewId) || !positiveVersion(input.expectedVersion)) return invalid("Interview hoặc version không hợp lệ.");
...
export async function rescheduleConfirmedInterview(
  ...
  if (!UUID.test(input.interviewId) || !positiveVersion(input.expectedVersion) || !UUID.test(input.idempotencyKey) || !input.startAt || !input.endAt || !input.interviewFormatId) return invalid("Thông tin xếp lại lịch chưa hợp lệ.");
  return withPermission(deps, ["interviews.manage"], (ctx) =>
...
export async function copyInterviewSchedule(
  ...
  if (![input.sourceInterviewId, input.targetApplicationId, input.expectedTargetRoundId, input.idempotencyKey].every((value) => UUID.test(value)) || !positiveVersion(input.expectedSourceVersion) || !positiveVersion(input.expectedTargetApplicationVersion) || !positiveVersion(input.expectedTargetRoundVersion) || !input.participantAppUserIds.every((id) => UUID.test(id))) return invalid("Copy payload không hợp lệ.");
  return withPermission(deps, ["interviews.manage"], (ctx) =>
    executeRpc(ctx, "copy_interview_schedule", {
      ...
      p_expected_target_application_version: input.expectedTargetApplicationVersion,
...
export async function reactivateApplication(
  ...
  if (!UUID.test(input.applicationId) || !positiveVersion(input.expectedVersion)) return invalid("Application hoặc version không hợp lệ.");
...
export async function deleteOrInactivateApplication(
  ...
  return withPermission(deps, ["applications.delete", "applications.manage"], (ctx) =>
    executeRpc(ctx, "delete_or_inactivate_application", { p_application_id: applicationId }),
    "any",
  );
}
...
export async function addInterviewParticipant(
  ...
  if (![input.interviewId, input.appUserId, input.idempotencyKey].every((value) => UUID.test(value))) return invalid("Participant payload không hợp lệ.");
  return withPermission(deps, ["interviews.participants", "interviews.view"], (ctx) =>
...
export async function removeInterviewParticipant(
  ...
  if (!UUID.test(input.interviewParticipantId) || !positiveVersion(input.expectedVersion)) return invalid("Participant hoặc version không hợp lệ.");
  return withPermission(deps, ["interviews.participants", "interviews.view"], (ctx) =>
...
export async function readdInterviewParticipant(
  ...
  if (!UUID.test(input.interviewParticipantId) || !UUID.test(input.idempotencyKey) || !["RESTORE_OLD_REPORT", "CREATE_NEW_REPORT"].includes(input.restoreMode)) return invalid("Re-add payload không hợp lệ.");
  return withPermission(deps, ["interviews.participants", "interviews.view"], (ctx) =>
...
export async function reorderInterviewParticipants(
  input: { interviewId: string; participantIds: string[]; expectedVersions: number[] },
  deps: InterviewCommandDeps = {},
) {
  if (!UUID.test(input.interviewId) || input.participantIds.length !== input.expectedVersions.length || !input.participantIds.every((id) => UUID.test(id)) || !input.expectedVersions.every(positiveVersion)) return invalid("Thứ tự Participant hoặc version không hợp lệ.");
  return withPermission(deps, ["interviews.participants", "interviews.view"], (ctx) =>
