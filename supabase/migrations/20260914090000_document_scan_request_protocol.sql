...
-- Session cancellation must fence all not-yet-staged scan work.
create or replace function private.cancel_document_scan_requests_for_session(p_session_id uuid)
returns void language plpgsql security definer set search_path='' as $$
begin
 update public.document_scan_requests set status_code='CANCELLED',leased_until=null,updated_at=clock_timestamp()
 where candidate_form_session_id=p_session_id and staged_change_id is null and status_code in ('PENDING','PROCESSING','ERROR','CLEAN');
end;
$$;
