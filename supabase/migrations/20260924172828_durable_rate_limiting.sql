...
begin
  ...
  loop
    ...
    v_window_started_at := pg_catalog.to_timestamp(
      pg_catalog.floor(
        pg_catalog.extract(epoch from p_now) / v_window_seconds
      ) * v_window_seconds
    );
    ...
    if v_current_count >= v_limit then
      v_retry_after := pg_catalog.greatest(
        1,
        pg_catalog.ceil(
          pg_catalog.extract(epoch from (v_window_ends_at - p_now))
        )::integer
      );
      v_max_retry_after := pg_catalog.greatest(v_max_retry_after, v_retry_after);
  ...
  loop
    ...
    v_rule_results := v_rule_results || pg_catalog.jsonb_build_array(
      pg_catalog.jsonb_build_object(
        ...
        'limit', v_limit,
        'windowSeconds', v_window_seconds,
        'remaining', pg_catalog.greatest(0, v_limit - v_current_count),
        'resetAt', v_window_ends_at
      )
    );
  end loop;
