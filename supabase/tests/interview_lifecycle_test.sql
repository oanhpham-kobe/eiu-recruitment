...
begin
  ...
  -- 13. Confirmed reschedule semantics were verified above: conflict rollback and AWAITING transition.
  select version_no into v from public.interviews where public.interviews.interview_id=test.interview_id; r:=public.delete_or_inactivate_interview(interview_id,v); assert (r->>'success')::boolean,'latest round is inactivated';
  ...
  update public.upload_reservations set status_code='VALIDATED',malware_scan_status='CLEAN' where upload_reservation_id=reservation_id;
  ...
  select count(*) into n from public.interview_documents where logical_document_id=logical_id; assert n=1,'participant can select interview documents'; execute 'reset role';
  ...
  -- 18. Bulk status change is all-or-nothing when an inactive participant exists.
  perform set_config('request.jwt.claims',jsonb_build_object('sub',hr_auth::text)::text,true);
  update public.app_users set is_active=false where app_user_id=i2;
  select version_no into v from public.interviews where public.interviews.interview_id=test.interview_id;
  r:=public.bulk_change_interview_schedule_status(array[interview_id],'AWAITING',array[v]); assert (r->>'error_code')='CURRENT_PARTICIPANT_INACTIVE_REASSIGN_REQUIRED','bulk status fails all-or-nothing on inactive participant';
  assert (select schedule_status_code from public.interviews where public.interviews.interview_id=test.interview_id)='CANCELLED','failed bulk leaves target unchanged';
  update public.app_users set is_active=true where app_user_id=i2;
