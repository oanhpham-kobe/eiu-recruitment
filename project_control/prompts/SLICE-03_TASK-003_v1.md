...
BASE_HEAD: 05e8d06bd7f11e091f02bcb37e81da4511ce0cbc
```
...
The executor MUST independently read every listed skill before implementation and record a receipt in the task evidence with `availability`, `loaded`, `applied`, and concrete `applied_to` values. Do not use the mere declaration above as the receipt.
...
12. Assert exact audit persistence: one `activity_log` row per changed entity and one `security_audit_log` batch event with the exact action code, server-derived actor identities, request ID, metadata keys, and value shapes above.
