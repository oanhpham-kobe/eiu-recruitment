...
### 1B. Multi-Agent & Worktree Ownership Contract
- **ONE WRITER PER WORKTREE:** Orca owns worktrees, handoffs, and DAGs. OMP owns in-worktree execution. Never allow concurrent writing agents in the same checkout.

### 1C. Graph Intelligence & Freshness Contracts (v2.4)
- **Direct Source + LSP:** Default local implementation path.
- **CRG (2.3.8):** Broad discovery and diff triage only (restricted tools).
- **GitNexus (1.6.10):** Precise symbol, caller/callee, and blast-radius analysis.
- **Freshness Gate:** Verify freshness before first task query. Re-verify after material source changes.
- **Database Authority:** Schema + migrations + direct SQL + tests. Graphs are non-authoritative for database behavior.
...
tdd (strict only for security/config/migration invariants)
accessibility for base shell
```
