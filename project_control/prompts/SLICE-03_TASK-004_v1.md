...
GOVERNANCE:
  SKILLS_REQUIRED:
    ...
    - react-testing
  SKILLS_RESOLVED:
    - react-patterns (.agents/skills/react-patterns)
    - vercel-react-best-practices (.agents/skills/vercel-react-best-practices)
    - accessibility (.agents/skills/accessibility)
    - react-testing (.agents/skills/react-testing)
...
- Status always has a textual label; badges use approved 16px semantic contrast colors and `--badge-width-submission` behavior. A read-only badge is not a button. Selection UI does not claim a mutation is available; selected-count feedback may be presented as non-mutating context.
